...
#define engine_bulid std::wstring(L"0.8.2 (pre-release)");
...
struct PahomEngineStruct {
    //
    std::string sBuild = "0.8.2 (pre-release)";
    std::string sBuildGame = "0.8.2 (pre-release)";
    //
    std::wstring sWBuild = L"0.8.2 (pre-release)";
    std::wstring sWBuildGame = L"0.8.2 (pre-release)";
    //
    bool CVsync = true;
    uint64_t fCPoint = 0;
    int64_t fStep = 13;
...
void  PahomEngineStruct::log(std::string text,int iLogTypeFlags = 1) {
    ...
    typeCurrent = (iLogTypeFlags == type::WARN ? typeStr[0] : typeStr[1]);
    typeCurrent = (iLogTypeFlags == type::INFO ? typeStr[1] : typeStr[1]);
    typeCurrent = (iLogTypeFlags == type::ERR  ? typeStr[2] : typeStr[1]);
    ImVec4 Time = {
      (float)local_tm->tm_hour,      // 0–23
      (float)local_tm->tm_min,       // 0–59
      (float)local_tm->tm_sec,       // 0–59
      (float)(now.time_since_epoch().count() % 10000000 / 10000.0f) // миллисекунды (примерно)
    };
    if (bLogEnabled) {
